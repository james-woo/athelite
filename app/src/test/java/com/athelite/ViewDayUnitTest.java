...
public class ViewDayUnitTest {

    private ViewDay _activity;
    private Date _date;

    @Before
    public void view_day_setup()  {
        ...
        intent.putExtra("VIEW_DAY_PARENT", "ViewDay");
        intent.putExtra("VIEW_DAY_DATETIME", 0);
        intent.putExtra(WorkoutPlanAdapter.WORKOUT_PLAN, JsonSerializer.workoutPlanToJson(workoutPlan));

        _activity = Robolectric.buildActivity(ViewDay.class)
                .withIntent(intent)
                .create()
                .get();
    }
    ...
    @Test
    public void view_day_activity_starts_test() throws Exception {
        assertTrue(_activity != null);
    }

    @Test
    public void view_day_toolbar_not_null_test() throws Exception {
        Toolbar toolbar = (Toolbar) _activity.findViewById(R.id.view_day_toolbar);
        assertNotNull(toolbar);
        assertNotNull(_activity.getSupportActionBar());
    }

    @Test
    public void view_day_toolbar_is_null_test() throws Exception {
